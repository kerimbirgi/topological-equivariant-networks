...
def main(cfg: DictConfig):
    ...
    # Get train/validation splits using OOD protein-based splitting
    if SPLIT_OOD:
        # Check if saved splits exist
        train_split_path = os.path.join(cfg.dataset.splits_dir, 'pretrain_train_ood_indices.npy')
        val_split_path = os.path.join(cfg.dataset.splits_dir, 'pretrain_val_ood_indices.npy')
        
        if os.path.exists(train_split_path) and os.path.exists(val_split_path):
            logger.info("Loading existing OOD protein splits...")
            train_indices = np.load(train_split_path)
            val_indices = np.load(val_split_path)
            train_subset = dataset.index_select(train_indices)
            valid_subset = dataset.index_select(val_indices)
            logger.info(f"Loaded splits: train={len(train_subset)}, val={len(valid_subset)}")
        else:
            logger.info("Creating new OOD protein splits...")
            train_subset, valid_subset, train_indices, val_indices = make_protein_ood_split(dataset, val_frac=0.2, seed=cfg.seed)
            
            # Save the indices for future use
            os.makedirs(cfg.dataset.splits_dir, exist_ok=True)
            
            np.save(train_split_path, train_indices)
            np.save(val_split_path, val_indices)
            logger.info(f"Saved OOD splits to {cfg.dataset.splits_dir}")
    else:
        # deterministic 80/20 split (fallback)
        ...
        train_subset = dataset.index_select(train_idx)
        valid_subset = dataset.index_select(val_idx)
